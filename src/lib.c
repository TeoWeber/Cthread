...
int cscheduler()
{
    ...
    if (tcb->state == PROCST_EXEC)
    {
        ...
        if (tcb->d_tid != -1)
        { // Libera a thread bloqueada em cjoin(), se existir
...
int cjoin(int tid)
{
    ...
    if (filaIndicator == THREAD_NOT_FOUND_ERROR)
    { // cfind_thread() retornou erro
        return THREAD_NOT_FOUND_ERROR;
    }
    else if (filaIndicator == IN_RUNNING_QUEUE)
    { // cfind_thread() retornou a própria thread executando, e a cjoin não pode bloquear a si mesmo
        return IN_RUNNING_QUEUE_ERROR;
    }
    else if (filaIndicator == IN_READY_QUEUE)
    { // cfind_thread() retornou que a thread esta na ready_queue
        filaRef = &ready_queue;
    }
    else // if (filaIndicator == IN_BLOCKED_QUEUE) // vai sempre ter fallback pra isso
    {    // cfind_thread() retornou que a thread esta na blocked_queue
        filaRef = &blocked_queue;
    }

    candidate_thread = cpop_thread(filaRef, tid, 0); // Pop da thread na sua respectiva lista

    if (candidate_thread->d_tid != -1)
    { // Se a thread já esta bloqueando, retorna THREAD_ALREADY_BLOCKING_ERROR
